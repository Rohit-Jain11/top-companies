...
export const addCompanyDetail = asyncHandler(async (req: Request, res: Response) => {
  const { url, page, perPage } = req.body;
  const result = await companiesService.addCompanyDetail(url, page, perPage, req.admin!.id);
  return sendSuccess(res, result, { message: "Companies imported successfully", statusCode: 201 });
});
