                    ...
                    <div className='text-center mb-8 xl:mb-15'>
                        <ul className='flex items-center border border-border rounded-2xl p-2.5 max-w-fit mx-auto overflow-x-auto bg-white'>
                            {categories.map((category, index) => (
                                <li 
                                    key={category}
                                    className={`relative ${index !== categories.length - 1 ? 'after:content-[""] after:absolute after:top-1/2 after:right-0 after:-translate-y-1/2 after:w-0.5 after:h-8 after:bg-border' : ''}`}
                                >
                                    <button type='button' className={`text-base leading-5.5 min-w-27.5 lg:min-w-38 px-2.5 lg:px-4 py-2.5 rounded-2xl cursor-pointer ${index === 0 ? 'text-white bg-foreground' : 'text-secondary'}`}>{category}</button>
                                </li>
                            ))}
                        </ul>
                    </div>
                    <div className='flex gap-6 xl:gap-15 flex-wrap'>
                        <div className='lg:basis-[65%] xl:basis-[66.1%] max-w-full w-full lg:max-w-[65%] xl:max-w-[66.1%] shrink-0'>
                                ...
                                {blogs.map((blog) => (
                                    <article key={blog.id} className='space-y-6'>
                                        ...
                                        <div className=''>
                                            ...
                                            <h2 className='font-fraunces text-foreground text-[32px] xl:text-[54px] leading-10 xl:leading-14.25 tracking-[-1.62px] font-normal'><Link href={`/blog/${blog.slug}`} className='block'>{blog.title}</Link></h2>
                                            <p className='text-lg leading-7 text-secondary mt-4 xl:mt-6 mb-4 xl:mb-7.5'>{blog.description}</p>
                                            <Link href={`/blog/${blog.slug}`} className='text-lg leading-7 font-semibold text-foreground py-2.75 px-6.5 inline-block border border-foreground rounded-full bg-white transition-all ease-in-out duration-300 hover:bg-primary hover:border-primary hover:text-white'>Read more</Link>
                                        </div>
                                    </article>
                                ))}
                            </div>
                            <ul className='flex flex-wrap justify-center gap-3.5 mt-8 xl:mt-15'>
                                <li>
                                    <button type='button' className='w-13 h-13 border border-border rounded-full flex items-center justify-center cursor-pointer'>
                                ...
                                <li>
                                    <button type='button' className='w-13 h-13 border rounded-full flex items-center justify-center cursor-pointer bg-foreground border-foreground text-white'>1</button>
                                </li>
                                <li>
                                    <button type='button' className='w-13 h-13 border border-border rounded-full flex items-center justify-center cursor-pointer text-foreground'>2</button>
                                </li>
                                <li>
                                    <button type='button' className='w-13 h-13 border border-border rounded-full flex items-center justify-center cursor-pointer'>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                            <path fillRule="evenodd" clipRule="evenodd" d="M16.2755 11.4745C16.4161 11.6151 16.4951 11.8058 16.4951 12.0047C16.4951 12.2036 16.4161 12.3943 16.2755 12.535L8.77553 20.035C8.63408 20.1716 8.44462 20.2472 8.24798 20.2455C8.05133 20.2438 7.86322 20.1649 7.72416 20.0258C7.58511 19.8868 7.50623 19.6987 7.50452 19.502C7.50281 19.3054 7.57841 19.1159 7.71503 18.9745L14.69 11.9995L7.71503 5.02445C7.57841 4.883 7.50281 4.69355 7.50452 4.4969C7.50623 4.30025 7.58511 4.11214 7.72416 3.97309C7.86322 3.83403 8.05133 3.75516 8.24798 3.75345C8.44463 3.75174 8.63408 3.82733 8.77553 3.96395L16.2755 11.464L16.2755 11.4745Z" fill="#111827"/>
                                        </svg>
                                    </button>
                                </li>
                            </ul>
                                    ...
                                    {recentPosts.map((post) => (
                                        <Link href={`/blog/${post.slug}`} key={post.id} className='rounded-[20px] p-2.5 blogGlass flex flex-wrap gap-4'>
                                            ...
                                            <div className='basis-[calc(58.6%-16px)] max-w-[calc(58.6%-16px)] shrink-0 self-center'>
                                                ...
                                                <h2 className='text-foreground text-base leading-5.5 font-semibold'>{post.title}</h2>
                                            </div>
                                        </Link>
                                    ))}
                                </div>
