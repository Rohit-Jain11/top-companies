...
import { Plus, RotateCcw, Star, Trash2, CheckCircle2, XCircle, Info } from "lucide-react";
...
export default function CompaniesPage() {
  ...
  const [metadataTarget, setMetadataTarget] = useState<Company | null>(null);
    ...
    <div className="space-y-4">
      ...
      <Dialog open={!!metadataTarget} onOpenChange={(open) => !open && setMetadataTarget(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{metadataTarget?.name} - Metadata</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4 text-sm">
            <div className="grid grid-cols-4 items-center gap-4">
              <span className="font-semibold">Tagline</span>
              <span className="col-span-3 text-muted-foreground">{metadataTarget?.detail?.tagline || "—"}</span>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <span className="font-semibold">Sales Email</span>
              <span className="col-span-3 text-muted-foreground">{metadataTarget?.detail?.salesEmail || "—"}</span>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <span className="font-semibold">Contact Phone</span>
              <span className="col-span-3 text-muted-foreground">{metadataTarget?.detail?.contactPhone || "—"}</span>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <span className="font-semibold">Min. Project Size</span>
              <span className="col-span-3 text-muted-foreground">{metadataTarget?.detail?.minProjectSize || "—"}</span>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <span className="font-semibold">Agency Status</span>
              <span className="col-span-3 text-muted-foreground">{metadataTarget?.detail?.agencyStatus || "—"}</span>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <span className="font-semibold">Reviews</span>
              <span className="col-span-3 text-muted-foreground">{metadataTarget?.detail?.reviews || "—"}</span>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <span className="font-semibold">UTM Source Enable</span>
              <span className="col-span-3 text-muted-foreground">
                {metadataTarget?.detail?.utmSourceEnable ? "Yes" : "No"}
              </span>
            </div>
          </div>
        </DialogContent>
